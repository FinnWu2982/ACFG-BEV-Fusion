...
from .lidar_corruption import apply_lidar_corruption
...
class DataProcessor(object):
    ...
    def _get_corruption_mode(self):
            """
            读取 corruption 模式:
            1. 如果不是训练模式 -> 读取 EVAL_CORRUPTION (默认 clean)
            2. 如果是训练模式:
            - 先看 ENABLE_MIX_TRAIN 是否为 1
                - 是 -> 50% 概率 clean, 50% 概率随机选 light/heavy
            - 否 -> 读取 TRAIN_CORRUPTION (默认 clean)
            """
            # 1. 测试/验证模式 (Eval)
            if not self.training:
                return os.getenv("EVAL_CORRUPTION", "clean")

            # 2. 训练模式 (Train) - Mix Training 逻辑
            # 只有显式设置 export ENABLE_MIX_TRAIN=1 时才生效
            if os.getenv("ENABLE_MIX_TRAIN", "0") == "1":
                # 50% 概率 Clean
                if np.random.rand() < 0.5:
                    return "clean"
                else:
                    # 50% 概率腐蚀 (随机 Light 或 Heavy)
                    # 你也可以改成 return "light" 或者 return "heavy" 固定一种
                    # 这里随机选是为了让模型见识更多样的情况
                    return np.random.choice(["light", "heavy"])

            # 3. 训练模式 - 普通指定 (如 export TRAIN_CORRUPTION=heavy)
            return os.getenv("TRAIN_CORRUPTION", "clean")

    def lidar_corruption(self, data_dict=None, config=None):
        """
        - clean: 完全 no-op，保持与官方原版一致
        - light/heavy: 调用 apply_lidar_corruption，对点云做腐蚀
        """
        if data_dict is None:
            return partial(self.lidar_corruption, config=config)

        mode = self._get_corruption_mode()
        if mode == "clean":
            # 干净：不动数据，保证和原版一模一样
            return data_dict
        ...
        ops_str = os.getenv("LIDAR_OPS", "dropout,fog,sector,smear")
        ...
        pts_np = apply_lidar_corruption(pts_np, mode, ops)

        if is_tensor:
            data_dict['points'] = torch.from_numpy(pts_np).to(pts.device)
        else:
            data_dict['points'] = pts_np
